...
The axioms  for probability includes three standard equalities (each of which is given officially as two inequalities), and one Horn axiom, which  is separated out for the sake of Proposition~\ref{proposition:horn} below.
...
\begin{align*}
T_\prang ~ = ~ & \text{Prob}, \, \text{Nondet}, \, \text{Ang},\, \text{Dist} \enspace , \\
T_\prdem ~ = ~ & \text{Prob}, \, \text{Nondet}, \, \text{Dem},\, \text{Dist} \enspace .
\end{align*}
...
To end the section, we observe that the Horn axiom in Figure~\ref{fig:axiomsmixed} can be replaced with an equational axiom, albeit one involving an infinitary expression.

\newcommand{\Iterx}[1]{(1\!-\!2^{- #1})x}

\begin{definition}
    \label{def:probaApproxConstruct}
    Let $x$ be a variable. For each $n \in \mathbb{N} \cup \{\infty\}$, we define an expression $\Iterx{n}$ inductively by
    $\Iterx{0} = \bot$ and $\Iterx{(n+1)}  = x\,  \prEff \,\Iterx{n}$. The
    expression $\Iterx{\infty}$ is defined as $\bigsqcup_n \Iterx{n}$.
\end{definition}

\begin{proposition}%[Removing Horn-Clauses]
\label{proposition:horn}
    For any theory containing the Bot and Prob axioms, an admissible model satisfies the Horn axiom if and only if it satisfies the equation 
    $\Iterx{\infty}= x$.
\end{proposition}
